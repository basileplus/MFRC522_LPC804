...
{	
  ...
  LPC_SPI0->TXCTL &= ~(SPI_CTL_EOT);                 // Start a new transfer, clear the EOT bit	
	...
	if ( WrLen == 1 ) {
		while ( (LPC_SPI0->STAT & SPI_STAT_TXRDY) == 0 );
		LPC_SPI0->TXDATCTL = SPI_CTL_LEN(DATA_WIDTH) | SPI_CTL_EOF | SPI_CTL_RXIGNORE |  *WrBuf;
		...
		*RdBuf = LPC_SPI0->RXDAT;
		...
		while (i <WrLen) {
			if ( i == 0 ) {
				LPC_SPI0->TXDATCTL = SPI_CTL_LEN(DATA_WIDTH)| SPI_CTL_RXIGNORE | *WrBuf++;
			}
			else if ( i == WrLen-1 ) {
				LPC_SPI0->TXDATCTL = SPI_CTL_LEN(DATA_WIDTH)| SPI_CTL_RXIGNORE | SPI_CTL_EOF | *WrBuf++;
			}
			else {
				LPC_SPI0->TXDAT = *WrBuf++;
			}
